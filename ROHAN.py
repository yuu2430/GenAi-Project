...
# ══════════════════════════════════════════════════════════════
    ...
    # Build a representative Decision Tree with simulated feature data
    np.random.seed(99)
    n_samples = 221
    ct_score   = np.clip(np.concatenate([LOW_CT, MOD_CT, HIGH_CT]), 1, 5)
    ai_dep     = AI_DEP
    creativity = np.clip(np.random.normal(2.898, 0.892, n_samples), 1, 5)
    cog_off    = np.clip(np.random.normal(2.8, 0.75, n_samples), 1, 5)
    ai_freq    = np.random.choice([1,2,3,4,5], size=n_samples, p=[0.09,0.18,0.31,0.30,0.12])

    cgpa_all = np.concatenate([low_cgpa_sim, mod_cgpa_sim, high_cgpa_sim])
    def cgpa_to_div(c):
        if c >= 8.0: return "Distinction"
        elif c >= 6.0: return "First"
        elif c >= 5.0: return "Second"
        elif c >= 4.0: return "Third"
        else: return "Fail"
    divisions = [cgpa_to_div(c) for c in cgpa_all]

    X_tree = np.column_stack([ct_score, ai_dep, creativity, cog_off, ai_freq])
    le = LabelEncoder(); y_tree = le.fit_transform(divisions)
    feat_names = ["Critical Thinking","AI Dependency","Creativity","Cog. Offloading","AI Frequency"]
    ...
    def get_node_color(node_id, tree, le, class_colors_hex):
        class_idx = np.argmax(tree.value[node_id][0])
        class_name = le.classes_[class_idx]
        return class_colors_hex.get(class_name, "#aaaaaa")

    def hex_to_rgb(h):
        h = h.lstrip('#')
        return tuple(int(h[i:i+2],16)/255 for i in (0,2,4))

    # Draw tree manually using matplotlib
    from sklearn.tree import _tree as _sklearn_tree

    tree_ = dt_vis.tree_
    feature_name = [feat_names[i] if i != _sklearn_tree.TREE_UNDEFINED else "undefined" for i in tree_.feature]

    fig_dt, ax_dt = plt.subplots(figsize=(16, 7))
    ax_dt.set_xlim(0, 1); ax_dt.set_ylim(0, 1)
    ax_dt.axis('off')
    ...
    def get_positions(node_id, depth, x_min, x_max):
        x = (x_min + x_max) / 2
        y = 1.0 - depth * 0.22 - 0.10
        node_positions[node_id] = (x, y)
        if tree_.children_left[node_id] != _sklearn_tree.TREE_LEAF:
            mid = (x_min + x_max) / 2
            get_positions(tree_.children_left[node_id],  depth+1, x_min, mid)
            get_positions(tree_.children_right[node_id], depth+1, mid,   x_max)

    get_positions(0, 0, 0.0, 1.0)

    # Draw edges first
    for node_id, (x, y) in node_positions.items():
        lc = tree_.children_left[node_id]
        rc = tree_.children_right[node_id]
        if lc != _sklearn_tree.TREE_LEAF:
            lx, ly = node_positions[lc]
            rx, ry = node_positions[rc]
            ax_dt.plot([x, lx], [y-0.035, ly+0.035], color="#b0bec5", lw=1.5, zorder=1)
            ax_dt.plot([x, rx], [y-0.035, ry+0.035], color="#b0bec5", lw=1.5, zorder=1)
            # Edge labels
            ax_dt.text((x+lx)/2-0.01, (y+ly)/2, "≤", fontsize=9, color=C["green"], ha='right', fontweight='bold')
            ax_dt.text((x+rx)/2+0.01, (y+ry)/2, ">",  fontsize=9, color=C["red"],   ha='left',  fontweight='bold')

    # Draw nodes
    box_w, box_h = 0.095, 0.060
    for node_id, (x, y) in node_positions.items():
        clr = get_node_color(node_id, tree_, le, class_colors_hex)
        rgb = hex_to_rgb(clr)
        bg_color = (rgb[0]*0.18+0.82, rgb[1]*0.18+0.82, rgb[2]*0.18+0.82)
        is_leaf = (tree_.children_left[node_id] == _sklearn_tree.TREE_LEAF)

        rect = plt.Rectangle((x-box_w/2, y-box_h/2), box_w, box_h,
                              facecolor=bg_color, edgecolor=clr, linewidth=2.0,
                              zorder=2, transform=ax_dt.transData, clip_on=False,
                              linestyle="--" if is_leaf else "-")
        ax_dt.add_patch(rect)

        class_idx = np.argmax(tree_.value[node_id][0])
        class_name = le.classes_[class_idx]
        samples = int(tree_.n_node_samples[node_id])
        gini = round(tree_.impurity[node_id], 3)

        if not is_leaf:
            feat = feature_name[node_id]
            threshold = round(tree_.threshold[node_id], 2)
            lines = [f"{feat}", f"≤ {threshold}", f"Gini: {gini}", f"n = {samples}", f"Class: {class_name}"]
        else:
            lines = [f"LEAF", f"Gini: {gini}", f"n = {samples}", f"Class: {class_name}"]

        line_h = 0.011
        total_h = len(lines) * line_h
        for i, line in enumerate(lines):
            ty = y + total_h/2 - i*line_h - line_h/2
            weight = 'bold' if (i == 0 or i == len(lines)-1) else 'normal'
            color = clr if (i == 0 or i == len(lines)-1) else C["slate"]
            ax_dt.text(x, ty, line, ha='center', va='center', fontsize=7.2,
                       color=color, fontweight=weight, zorder=3)

    # Legend
    legend_x, legend_y = 0.01, 0.08
    ax_dt.text(legend_x, legend_y + 0.045, "Class Legend:", fontsize=9, color=C["slate"], fontweight='bold')
    for i, (cls, clr) in enumerate(class_colors_hex.items()):
        rect = plt.Rectangle((legend_x + i*0.13, legend_y), 0.02, 0.02,
                              facecolor=clr, edgecolor='none', zorder=3)
        ax_dt.add_patch(rect)
        ax_dt.text(legend_x + i*0.13 + 0.025, legend_y + 0.010, cls, fontsize=8, color=C["slate"], va='center')

    ax_dt.set_title("Decision Tree — Top 3 Levels (max_depth=3)", fontsize=13, fontweight='bold',
                    color=C["ink"], pad=10)
    plt.tight_layout()
    st.pyplot(fig_dt, use_container_width=True); plt.close()
