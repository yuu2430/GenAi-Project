...
# ══════════════════════════════════════════════════════════════
    ...
    for nid, (x, y) in node_positions.items():
        ...
        if lc != _sklearn_tree.TREE_LEAF:
            ...
            fig_dt.add_trace(go.Scatter(
                x=[(x+rx)/2 + 0.02], y=[(y+ry)/2],
                mode="text", text=[">"],
                textfont=dict(size=13, color="#c0392b", family="Inter"),
                hoverinfo="skip", showlegend=False
            ))
    ...
    for nid, (x, y) in node_positions.items():
        ...
        gini       = round(tree_.impurity[nid], 3)
        is_leaf    = (tree_.children_left[nid] == _sklearn_tree.TREE_LEAF)
        pal        = CLASS_PALETTE[class_name]
        class_dist = tree_.value[nid][0].astype(int).tolist()
        class_dist_str = " | ".join(
            f"{le.classes_[i]}: {class_dist[i]}" for i in range(len(class_dist))
        )

        if is_leaf:
            label_html = (f"<b>🍃 LEAF</b><br>"
                          f"Class: <b>{class_name}</b><br>"
                          f"Gini: {gini}   n = {samples}")
            hover_txt  = (f"<b>Leaf Node</b><br>"
                          f"Predicted Class: <b>{class_name}</b><br>"
                          f"Samples: {samples}<br>"
                          f"Gini Impurity: {gini}<br>"
                          f"<br><b>Class Distribution</b><br>{class_dist_str}")
        else:
            feat      = feature_name[nid]
            threshold = round(tree_.threshold[nid], 2)
            label_html = (f"<b>{feat}</b><br>"
                          f"≤ {threshold}<br>"
                          f"Gini: {gini}   n = {samples}<br>"
                          f"↳ <b>{class_name}</b>")
            hover_txt  = (f"<b>Split Node</b><br>"
                          f"Feature: <b>{feat}</b><br>"
                          f"Threshold: ≤ {threshold}<br>"
                          f"Samples: {samples}<br>"
                          f"Gini Impurity: {gini}<br>"
                          f"Majority Class: <b>{class_name}</b><br>"
                          f"<br><b>Class Distribution</b><br>{class_dist_str}")
        ...
        fig_dt.add_trace(go.Scatter(
            ...
            marker=dict(
                symbol="square",
                size=52 if not is_leaf else 46,
                color=pal["fill"],
                line=dict(color=pal["border"], width=3),
                opacity=0.93,
            ),
            name=class_name,
        ...
        fig_dt.add_trace(go.Scatter(
            ...
            mode="text",
            text=[label_html],
            textfont=dict(size=9.5, color=pal["text"], family="Inter"),
    ...
    fig_dt.update_layout(
        title=dict(
            text="<b>Decision Tree Classifier — Top 3 Levels (max_depth = 3)</b>",
            font=dict(size=15, color="#0d1b2a", family="Libre Baskerville, Georgia, serif"),
        ...
        legend=dict(
            title=dict(text="<b>Class Legend</b>", font=dict(size=11, color="#0d1b2a")),
            orientation="h",
