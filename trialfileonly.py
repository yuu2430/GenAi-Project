...
def plotly_defaults(fig, h=420):
    ...
    return fig
def show_snippet(columns, n=5):
    if df is not None:
        try:
            snippet = df[columns].dropna().sample(n, random_state=1)
...
def load_main_data():
    try:
        df = pd.read_excel(GITHUB_DATA_URL)
        return df
    except:
        return None
...
# ══════════════════════════════════════════════════════════════
    ...
    if sub == "Gender-wise Analysis":
        ...
        show_snippet(["Gender", "AI Dependency Score"])
    ...
    # ── LEVEL OF STUDY ──
        ...
        step("Step 1 — Data Snippet")
        show_snippet(["Level_of_study", "AI Dependency Score"])
    ...
    # ── SCHOOLING + FACULTY ──
        ...
        with tab_s:
            step("Step 1 — Data Snippet")
            show_snippet(["Faculty", "AI Dependency Score"])
    ...
    # ── OVERALL DEPENDENCY ──
        ...
        st.latex(r"W = \frac{\left(\sum_{i=1}^n a_i x_{(i)}\right)^2}{\sum_{i=1}^n (x_i - \bar{x})^2}")
        st.markdown(f"<div style='font-size:13px; color:{C['muted']}; margin-bottom:16px;'>"
            "W = test statistic · x_(i) = ordered sample values · x̄ = sample mean · aᵢ = constants · n = sample size"
            "</div>", unsafe_allow_html=True)
        c1,c2,c3 = st.columns(3)
